...
def save_best_patches(file_name, image, im_patches, class_freq, data_dir, folder_names = ['background', 'single_ac', 'single_tc']):
    patch_size = im_patches.shape[3]
    stride = im_patches.shape[1]
    setname = "patches/"
    paths = [os.path.join(data_dir+setname+folder_name+'/') for folder_name in folder_names]
    ...
    n_patches = 5 # number of patches to save for each class
    idx = np.empty((len(folder_names), n_patches), dtype=int)
    for i in range(len(folder_names)):
        idx[i,:] = np.argsort(class_freq[:,i])[::-1][:n_patches]
...
def process_single_image(file_name, image, patch_size, stride, vars, data_dir, folder_names):
    im_patches = patch_image(image, patch_size, stride, vars)
    class_freq = calc_class_freq(im_patches)
    save_best_patches(file_name, image, im_patches, class_freq, data_dir, folder_names)
    return None
...
def process_all_images(data_path, patch_size, stride, vars, data_dir, folder_names):
    ...
    file_names = [p[-20:] for p in tqdm(single_file_paths)]

    for i, image in enumerate(tqdm(data)):
        file_name = file_names[i][:-3]
        process_single_image(file_name, image, patch_size, stride, vars, data_dir, folder_names)
    return None

if __name__ == "__main__":
    data_path = "C:/Users/amali/Desktop/AI4Good Local folder/data/raw/test/"
    patch_size = 200
    stride = 100
    vars = ['Z1000', 'U850', 'V850']
    data_dir = "C:/Users/amali/Desktop/AI4Good Local folder/data/"
    folder_names = ['background', 'single_ac', 'single_tc']
    process_all_images(data_path, patch_size, stride, vars, data_dir, folder_names)