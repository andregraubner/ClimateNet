...
wandb_logger = WandbLogger(log_model="all", project="ai4good/vanilla_model")
...
class ImageDataset(Dataset):
    def __init__(self, setname, transform=None, target_transform=None):
        ...
        if self.setname == 'train':
            self.file_names = os.listdir(f'{setname}/{setname}/')

        if self.setname == 'val':
            self.file_names = os.listdir(f'train/test/')
        
        else:
            self.file_names = os.listdir(f'{setname}/')
